...
class MyDataset(Dataset):
    ...
    def __init__(self,
        ...
        # 1. 加载并预处理原始数据
        self.raw_data = data
        self.max_len = max(self.raw_data.groupby("主队").count()["id"])
...
class MyDataset2(Dataset):
    ...
    def _build_sequences(self):
        ...
        # 存储每支球队的历史比赛记录（按时间）
        processed_samples = []
        for idx,row in self.raw_data.sort_values("比赛时间", ascending=True).iterrows():
            team_a_id = self.team2id[row["主队"]]
            team_b_id = self.team2id[row["客队"]]
            # 保存处理后的样本
            processed_samples.append({
                "team_a_id": team_a_id,
                "team_b_id": team_b_id,
                "team_a_id_score": row["主队得分"],
                "team_b_id_score": row["客队得分"],
                "diff": row["主队得分"] - row["客队得分"],
                "label": row["label"],
            })
    ...
    def __getitem__(self, idx):
        ...
        return {
            ...
            "team_a_id_score": int(sample["team_a_id_score"]),
            "team_b_id_score": int(sample["team_b_id_score"]),
            "diff": sample["diff"],
            "label": sample["label"]
        }
